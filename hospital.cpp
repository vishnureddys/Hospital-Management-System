...
class PATIENT
{
	...
	private :
			...
			char name[26], address[36], phone[10], gen, disease[16], doctor[26] ;
} ;
...
// gotoxy(x,y) divides the console screen into 50 rows and 80 columns
...
void ROOM :: LIST(void)
{
	clrscr();
	int row = 4, flag ;
	int sta = 0 ; //To count the number of available beds
	char ch ;
	gotoxy(1,1) ;
	...
	while (file.read((char *) this, sizeof(ROOM)))
	{
		...
		if (row == 22)
		{
			flag = 1 ;
			gotoxy(1,25) ;
			cout <<"Press any key to continue, or ESC to break..." ;
			ch = getch() ;
			if (ch == 27)
				break ;
			row = 4 ;
...
void PATIENT :: ADMIT(void)
{
	...
	char t1[5], pname[26], paddress[36], pphone[10], pgen, pdisease[16], pdoctor[26], ch ;
	int d1, m1, y1, valid ;
	ROOM r ;
	rno = r.ROOMNO() ;
	if (rno == 0)
	...
	do
	{
		valid = 1 ;
		gotoxy(5,25) ; clreol() ;
		cout <<"ENTER PHONE NO. OF THE PATIENT or <ENTER> FOR NONE"<<endl ;
		clreol() ;
		gets(pphone) ;
		if (pphone[0] == '0')
			return ;
		if ((strlen(pphone) < 7 && strlen(pphone) > 0) || (strlen(pphone) > 10))
		{
			valid = 0 ;
			gotoxy(5,25) ; clreol() ;
	...
	{
		gotoxy(5,25) ; clreol() ;
		cout <<"ENTER GENDER OF THE PATIENT (M/F)"<<endl ;
		//gotoxy(30,7) ; 
		clreol() ;
		pgen = getche() ;
		pgen = toupper(pgen) ;
...
void PATIENT :: DISPLAY_RECORD(int rno, int bno)
{
	...
	while (file.read((char *) this, sizeof(PATIENT)))
	{
		if (rno == roomno && bno == bedno)
		{
			textcolor(BLACK) ; textbackground(WHITE) ;
			gotoxy(5,1) ;
			printf("Date of Admit : %d/%d/%d",dd,mm,yy) ;
			gotoxy(60,1) ;
			printf("Room no.: %d",roomno) ;
			gotoxy(60,2) ;
			printf("Bed no. : %d",bedno) ;
			textcolor(LIGHTGRAY) ; textbackground(BLACK) ;
			gotoxy(5,3) ;
			cout <<"Name    : " <<name ;
			gotoxy(5,4) ;
